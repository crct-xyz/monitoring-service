...
function parseLambdaEvent(event) {
  ...
  // Parse the inner body, which is a stringified JSON
  const bodyData = JSON.parse(parsedEvent.body);

  // Extract the first (and only) transaction from the body
  const transaction = bodyData[0];
...
exports.handler = async (event) => {
  ...
  // Checking to identify required transaction information and logging to console
